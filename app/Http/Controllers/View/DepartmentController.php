...
class DepartmentController extends Controller
{
    ...
    public function deleted(Request $request)
    {
        ...
        $forcedBanned = null;
        return view('pages.superadmin.departments', compact('deptStats', 'search', 'status', 'sort', 'free', 'bannedFilter', 'deleted', 'forcedBanned'));
    }
    protected function getByPlan(Request $request, string $plan, bool $free, bool $deleted, ?string $forcedBanned = null)
    {
        $user = $request->user();
        if (!in_array($user->role->name, ['superadmin'])) {
        ...
        $query = $this->buildDepartmentsQuery($plan, !$deleted);
        ...
        if ($free && $forcedBanned === null) {
            // Oddiy "free" sahifasi: banned bo'lganlarni chiqarmaymiz
            $query->whereRaw(
            ...
            // freeBanned() yoki ?banned=banned orqali kelgan holat
            $query->whereRaw(
                "(SELECT COUNT(*) FROM bans 
              WHERE bans.bannable_type = ? 
                AND bans.bannable_id = departments.id 
        ...
        // agar 'all' bo'lsa hech narsa qo'shmaymiz (lekin free uchun yuqoridagi shart ishlaydi)
        ...
        return view('pages.superadmin.departments', compact(
            'deptStats',
            'search',
            'sort',
            'free',
    ...
    protected function buildDepartmentsQuery(string $plan, bool $onlyNonDeleted = true)
    {
        $bannableType = addslashes(\App\Models\Department::class);
    ...
    public function upgrade(Request $request, $departmentId)
    {
        ...
        // agar ban mavjud bo‘lsa -> ban ni bekor qilamiz
        if ($dept->ban) {
            $dept->ban->update([
                'active' => false,
                'starts_at' => null,
    ...
    public function store(Request $request)
    {
        ...
        if ($plan == 'trial') {
            ...
            $startsAt = $validated['ban_starts_at'] ? Carbon::parse($validated['ban_starts_at']) : Carbon::now();
            $deptBan = Ban::create([
                'bannable_type' => Department::class,
                'bannable_id'   => $dept->id,
                'reason'        => 'plan',
