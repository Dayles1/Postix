<header
    ...
    <div class="flex flex-col items-center justify-between grow xl:flex-row xl:px-6">
        <div
            ...
            {{-- SUPERADMIN NAVIGATION (FREE + EXPIRED bitta switch ga aylantirildi) --}}
            

            {{-- Desktop Sidebar Toggle (xl va undan yuqori) --}}
                <button
            ...
            {{-- Mobile Sidebar Toggle (xl dan past) --}}
                <button
            ...
            {{-- Logo (dark mode qo'llab-quvvatlash bilan) --}}
            <a href="{{ route('departments.index') }}" class="xl:hidden">
                <img class="dark:hidden h-10 w-auto" src="/images/logo/logo.jpg" alt="Logo" />
                <img class="hidden dark:block h-10 w-auto" src="/images/logo/logo.jpg" alt="Logo" />
            </a>

            {{-- Application Menu Toggle (mobile) - eski SVG dots bilan --}}
        ...
        {{-- RIGHT SIDE (to'liq eski chiroyli style: language + theme + user) --}}
        <div :class="isApplicationMenuOpen ? 'flex' : 'hidden'" x-cloak
            ...
            <div class="flex items-center gap-2 2xsm:gap-3">
                ...
                <div class="relative" x-data="{ open: false }" @click.away="open=false">
                    <button @click="open = !open"
                        class="flex items-center justify-center h-11 px-3 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg dark:bg-gray-900 dark:text-gray-400 dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800">
                        {{ strtoupper(app()->getLocale()) }}
                        <svg class="w-4 h-4 ml-2" viewBox="0 0 20 20" fill="none">
                            <path d="M5 7l5 5 5-5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" />
                        </svg>
                    </button>

                    <div x-show="open" x-transition x-cloak
                        class="absolute left-0 xl:left-auto xl:right-0 mt-2 w-40 rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-800 dark:bg-gray-900 z-50">
                        <a href="{{ url('/lang/uz') }}"
            ...
            <div class="relative" x-data="{ open: false }" @keydown.escape="open = false" @click.away="open = false">
                <button @click.prevent="open = !open" type="button"
                    class="flex items-center gap-3 rounded-md px-2 py-1 hover:bg-gray-100 dark:hover:bg-white/3 focus:outline-none"
                    aria-haspopup="true" :aria-expanded="open.toString()">

                    @if (auth()->user()->avatar?->path)
                        <img src="{{ asset('storage/' . auth()->user()->avatar->path) }}"
                            alt="{{ auth()->user()->name ?? 'User' }}"
                            class="w-11 h-11 rounded-full object-cover border border-gray-200 dark:border-gray-700">
                    @else
                        <span
                            class="inline-flex items-center justify-center w-11 h-11 rounded-full bg-gradient-to-tr from-indigo-600 to-indigo-500 text-white font-semibold">
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    class="absolute right-0 mt-3 w-64 rounded-2xl border border-gray-200 bg-white shadow-lg dark:border-gray-800 dark:bg-gray-900 z-50">
                    <div class="p-3">
                        <div class="flex items-center gap-3 pb-3">
                            ...
                            @else
                                <div
                                    class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-tr from-indigo-600 to-indigo-500 text-white font-semibold text-lg">
                                    {{ strtoupper(substr(auth()->user()->name ?? 'U', 0, 1)) }}
                                </div>
                            @endif
                        ...
                        <ul class="py-2">
                            <li>
                                <a href="{{ route('users.profile', auth()->user()->id) }}" @click="open = false"
                                    class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-white/5">
                                    <svg class="w-5 h-5 text-gray-500" viewBox="0 0 24 24" fill="none">
                                        <path
                                            d="M12 12c2.485 0 4.5-2.015 4.5-4.5S14.485 3 12 3 7.5 5.015 7.5 7.5 9.515 12 12 12zM3 21c0-3.866 3.134-7 7-7h4c3.866 0 7 3.134 7 7"
                        ...
                        <div class="pt-3">
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <button type="submit" @click="open = false"
                                    class="flex items-center w-full gap-3 px-3 py-2 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-white/5">
