{{-- resources/views/pages/admin/catalog.blade.php --}}
@extends('layouts.app')
...
@section('content')
    ...
    @php
        ...
        $baseUrl = route('admin.catalogs.index');

        $indexRoute = route('admin.catalogs.index');
    ...
    <div class="mx-auto max-w-7xl p-4 space-y-6">
        ...
        <div class="space-y-4">
            ...
            @else
                <div class="bg-white border border-gray-200 rounded-3xl shadow dark:bg-gray-800 dark:border-gray-700 overflow-hidden">
                    <!-- Desktop table -->
                            ...
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-600 dark:text-gray-400">
                                        {{ __('messages.catalogs.table.catalog') }}
                                    </th>
                                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-600 dark:text-gray-400">
                                        {{ __('messages.catalogs.table.owner') }}
                                    </th>
                                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-600 dark:text-gray-400">
                                        {{ __('messages.catalogs.table.peers_count') }}
                                    </th>
                                    <th class="px-6 py-4 text-left text-xs font-medium text-gray-600 dark:text-gray-400">
                                        {{ __('messages.catalogs.table.created_at') }}
                                    </th>
                                    <th class="px-6 py-4 text-right text-xs font-medium text-gray-600 dark:text-gray-400">
                                        {{ __('messages.catalogs.table.actions') }}
                                    </th>
                                </tr>
                            </thead>
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <div class="text-right">
                                        <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                            {{ is_array($c->peers) ? count($c->peers) : 0 }}
                                            {{ __('messages.catalogs.peers_label') }}
                                        </div>
                                    </div>
                                </div>
    ...
    <div id="modalCreate" class="fixed inset-0 z-[99999] hidden flex items-center justify-center p-4 bg-black/60"
        ...
        <div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-h-[92vh] overflow-y-auto">
            ...
            <div class="p-6">
                ...
                <form id="formCreate" method="POST" action="{{ route('admin.catalogs.store') }}">
                    @csrf
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-medium mb-3">{{ __('messages.catalogs.field_peers_help') }}</label>
                    ...
                    <div class="flex gap-3 justify-end">
                        <button type="button" data-modal-close
                            class="px-8 py-4 border border-gray-300 dark:border-gray-600 rounded-3xl hover:bg-gray-50 dark:hover:bg-gray-700">
                            Bekor qilish
                        </button>
                        <button id="createSubmit" type="submit"
                            class="px-8 py-4 bg-green-600 hover:bg-green-700 text-white rounded-3xl">
                            Saqlash
                        </button>
                    </div>
                </form>
                ...
                <form id="formEdit" method="POST" action="#">
                    ...
                    <div class="flex gap-3 justify-end">
                        ...
                        <button id="editSubmit" type="submit"
                            class="px-8 py-4 bg-yellow-500 hover:bg-yellow-600 text-white rounded-3xl">
                            Yangilash
                        </button>
                    </div>
                </form>
    ...
    <div id="modalShow" class="fixed inset-0 z-[99999] hidden flex items-center justify-center p-4 bg-black/60"
        aria-hidden="true">
        <div class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-3xl shadow-2xl max-h-[92vh] overflow-y-auto">
            <button type="button" data-modal-close
                class="absolute top-5 right-5 text-4xl leading-none text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">×</button>
            ...
            <div class="p-6">
                ...
                <div class="mb-6">
                    ...
                    <div class="border border-gray-200 dark:border-gray-600 rounded-3xl p-5 bg-gray-50 dark:bg-gray-700 max-h-64 overflow-y-auto">
                ...
                <form id="formDelete" method="POST" action="#">
                    ...
                    <div class="flex gap-3 justify-center">
                        ...
                        <button id="deleteSubmit" type="submit"
                            class="px-8 py-4 bg-red-600 hover:bg-red-700 text-white rounded-3xl">
                            O‘chirish
                        </button>
                    </div>
                </form>
    ...
    <script type="module">
        ...
        async function request({
            ...
            return await requestJson(url, data, method);
        }
        ...
        function showToast(text = '', type = 'success', timeout = 3500) {
            ...
            el.className = 'pointer-events-auto px-6 py-4 rounded-3xl shadow-2xl text-white text-sm flex items-center gap-3';
        ...
        function createPeerElement(value) {
            ...
            div.className = 'peer-item flex items-center justify-between bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-2xl px-5 py-3.5 mb-3';
            ...
            removeBtn.className = 'text-red-600 hover:text-red-700 text-2xl leading-none w-8 h-8 flex items-center justify-center';
                ...
                try {
                    ...
                    if (err.status === 422 && err.errors) {
                        if (err.errors.title && errTitle) {
                            errTitle.textContent = Array.isArray(err.errors.title) ? err.errors.title.join(' ') : err.errors.title;
        ...
        if (formDelete) {
            formDelete.addEventListener('submit', async (ev) => {
                ...
                try {
                    const data = await requestJson(formDelete.action, {}, 'delete');
